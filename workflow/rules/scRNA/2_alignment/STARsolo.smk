...
rule STARsolo:
    input:
        fq1 = lambda w : [i.format(read='1') for i in sample_dict[w.sample]],
        fq2 = lambda w : [i.format(read='2') for i in sample_dict[w.sample]],
        idx = f"{idx}/",
        idx_files = [
            f"{idx}/Genome",
            f"{idx}/Log.out",
