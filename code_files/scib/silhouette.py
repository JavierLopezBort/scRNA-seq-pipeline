...
def silhouette_label(adata, label_key, embed, metric="euclidean", scale=True):
...
def silhouette_batch(adata, batch_key, embed, metric="euclidean", scale=True):
    ...
    sil = silhouette_samples(
            adata.obsm[embed], adata.obs[batch_key], metric=metric
        )
...
def silhouette_batch_label(
    ...
    **Examples**
    ...
    if len(sil_per_label) == 0:  
        ...
        for group in adata.obs[label_key].unique():
            sil_per_label.append((group, np.nan))
            
        sil_df = pd.DataFrame.from_records(
            sil_per_label, columns=["cell_annot_comb", batch_key]
        )
        sil_means = sil_df
        sil_means.set_index('cell_annot_comb', inplace=True)
        asw = np.nan
        ...
        sil_means = round(sil_df.groupby("cell_annot_comb").mean(), 2)
        asw = sil_means[batch_key].mean()

    if verbose:
        print(f"mean silhouette per group: {sil_means}")

    if return_all:
        return asw, sil_means

    return asw

def silhouette_label_batch(
    adata,
    batch_key,
    label_key,
    embed,
    ...
    for group in adata.obs[batch_key].unique():
        adata_group = adata[adata.obs[batch_key] == group]
        n_batches = adata_group.obs[label_key].nunique()

        if (n_batches == 1) or (n_batches == adata_group.shape[0]):
            continue
        
        sil = silhouette_score(
            X=adata_group.obsm[embed], labels=adata_group.obs[label_key], metric=metric
        )
    ...
    if len(sil_per_label) == 0:
        ...
        asw = sil_df["silhouette_score"].mean()

    if verbose:
        print(f"mean silhouette per group: {sil_df}")
